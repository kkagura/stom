...
export function lineLine(a1: number[], a2: number[], b1: number[], b2: number[]) {
...
export const rectRect = (r1: number[][], r2: number[][]) => {
  ...
  return r1.some((_, i) => lineRect(r1[i], r1[(i + 1) % l], r2)) || r2.some((_, i) => lineRect(r2[i], r2[(i + 1) % l], r1));
};
...
export const smoothPath = (path: number[][][], maxRadius: number) => {
  ...
  for (let i = 0; i < cloned.length - 1; i++) {
    ...
    const radius = Math.min(maxRadius, ...d.map(item => (Math.abs(item) - 2) / 2));
    ...
    result.push(`M ${current[0][0]} ${current[0][1]} L ${currentPoint[0]} ${currentPoint[1]}`);

    if (radius > 0) {
      result.push(`M ${currentPoint[0]} ${currentPoint[1]}  Q ${base[0]} ${base[1]}, ${nextPoint[0]} ${nextPoint[1]}`);
    }
  }

  const last = cloned[path.length - 1];

  result.push(`M ${last[0][0]} ${last[0][1]} L ${last[last.length - 1][0]} ${last[last.length - 1][1]}`);
