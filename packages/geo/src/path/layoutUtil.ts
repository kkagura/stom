...
export const checkDirectionIsValid = (from: number[], to: number[], direction: Direction) => {
...
export const checkCanFollowWaypoint = (
  ...
  const getCoord = (index: number) => (isH ? [coord[0], index] : [index, coord[1]]);
...
export const getBoxConstraintsInfo = (
  ...
  minDist: number,
  isCovered: boolean
): Array<PathFindingPointData & { boundaryBox?: number[][]; box?: number[][] }> => {
